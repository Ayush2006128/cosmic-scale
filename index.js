    ...
    function createProton() {
        ...
        for(let i=0; i<3; i++) {
            const q = new THREE.Mesh(quarkGeo, new THREE.MeshBasicMaterial({color: 0xffff00}));
            q.userData = { speed: Math.random()*2, axis: new THREE.Vector3(Math.random(), Math.random(), Math.random()).normalize() };
            q.name = "quark";
            group.add(q);
        }
        return group;
    ...
    function createAtom() {
        ...
        const nucleus = new THREE.Mesh(nGeo, new THREE.MeshBasicMaterial({color: 0xff3333}));
        group.add(nucleus);
        ...
        for(let i=0; i<3; i++){
            ...
            const elec = new THREE.Mesh(eGeo, new THREE.MeshBasicMaterial({color: 0x00ffff}));
    ...
    function createDNA() {
        ...
        for(let i=0; i<=steps; i++) {
            ...
            const atom1 = new THREE.Mesh(sphereGeo, new THREE.MeshLambertMaterial({color: 0xff0000}));
            atom1.position.set(x1, y, z1);
            
            const atom2 = new THREE.Mesh(sphereGeo, new THREE.MeshLambertMaterial({color: 0x0000ff}));
            atom2.position.set(-x1, y, -z1);
            
            const bar = new THREE.Mesh(barGeo, new THREE.MeshLambertMaterial({color: 0xcccccc}));
    ...
    function createBeachBall() {
        ...
        const geo = new THREE.SphereGeometry(1, 32, 32);
        const mat = new THREE.MeshStandardMaterial({ map: tex, roughness: 0.2, metalness: 0.1 });
    ...
    function createSun() {
        ...
        const sMat = new THREE.MeshBasicMaterial({ color: 0xffaa00, transparent: true, opacity: 0.2, blending: THREE.AdditiveBlending });
    ...
    function createSolarSystem() {
        const group = new THREE.Group();
        // Sun marker
        const sun = new THREE.Mesh(new THREE.SphereGeometry(0.1), new THREE.MeshBasicMaterial({color: 0xffff00}));
        group.add(sun);
    ...
    function createGalaxy() {
        ...
        const mat = new THREE.PointsMaterial({ size: 0.02, vertexColors: true, blending: THREE.AdditiveBlending });
    ...
    OBJECTS.forEach(objData => {
        ...
        switch(objData.type) {
            ...
            case 'virus': mesh = createVirus(); break;
            case 'ball': mesh = createBeachBall(); break;
            case 'earth': mesh = createEarth(); break;
            case 'sun': mesh = createSun(); break;
            case 'solar_system': mesh = createSolarSystem(); break;
            case 'galaxy': mesh = createGalaxy(); break;
            case 'universe': mesh = createUniverse(); break;
            default: mesh = new THREE.Mesh(new THREE.BoxGeometry(1,1,1), new THREE.MeshBasicMaterial({color:0xff00ff}));
        }
    ...
    for(let i=0; i<2000; i++) {
        starsPos.push((Math.random()-0.5)*200, (Math.random()-0.5)*200, (Math.random()-0.5)*200);
    }
    starsGeo.setAttribute('position', new THREE.Float32BufferAttribute(starsPos, 3));
    const stars = new THREE.Points(starsGeo, new THREE.PointsMaterial({color:0x888888, size: 0.5}));
    scene.add(stars);
        ...
        sceneObjects.forEach(obj => {
            ...
            if(obj.userData.type === 'proton') {
                obj.children.forEach(c => {
                    if(c.name==='quark') {
                        c.position.addScaledVector(c.userData.axis, Math.sin(Date.now()*0.005 + c.id)*0.01);
                    }
                });
            }
